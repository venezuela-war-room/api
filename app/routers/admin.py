...
router = APIRouter(
    ...
    responses={401: {"description": "Missing or incorrect `X-Master-Key`."}},
)
...
@router.post(
    ...
    description=(
        "Issue a new `X-Admin-Key` for a volunteer team. The raw key is returned **once** "
        "in the response and never stored — only its SHA-256 hash is persisted."
    ),
)
...
@router.get(
    ...
    description="List all issued keys with metadata. Raw keys and hashes are never returned.",
)
...
@router.delete(
    ...
    description="Deactivate a key by ID. Subsequent requests using it will be rejected.",
    responses={404: {"description": "No key with that ID exists."}},
)
